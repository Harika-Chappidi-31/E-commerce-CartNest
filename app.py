...
import uuid
import razorpay
client = razorpay.Client(auth=("rzp_test_SzpwcuFklB1Edt", "ziV1BGNR81FgKj0RvPTYEYYy"))
...
app = Flask(__name__)
app.wsgi_app=ProxyFix(app.wsgi_app,x_port=1,x_host=1)
    ...
    try:
        ...
        try:
            ...
            if not admin_details:
                return jsonify({'status': 'failed','message': 'Invalid token data'}), 400
        except Exception:
...
def adminlogin():
    ...
    try:
        ...
        if not admin_data:
            return jsonify({'status': 'failed','message': 'Invalid email or password' }), 404
...
def admindashboard():
    try:
        ...
        return jsonify({
            'status': 'success', 'message': 'Welcome to the admin dashboard' ,'admin':{ 'adminid': session.get('adminid'), 'adminemail': session.get('adminemail') }}), 200
    except Exception as e:
...
def additem():
    ...
    try:
        ...
        try:
            ...
            return jsonify({ 'status': 'failed', 'message': 'Quantity must be an integer and price must be a number'}), 400
...
def viewallitems():
    ...
    try:
        ...
        return jsonify({'status': 'success', 'item': products}), 200
    except Exception as e:
...
def updatecart():
    ...
    try:
        ...
        if not stock_item:
            return jsonify({'status':'failed','message':'Item not found'}),404
        available_stock=int(stock_item[0])
        current_qty=int(cart_item[0])
    ...
    try:
        ...
        for item in cart_items:
            ...
            return jsonify({
                ...
                'razorpay_key': 'rzp_test_SzpwcuFklB1Edt'
            }), 200
...
def verify_payment():
    ...
    try:
        ...
        #----------------- Login validation------------------
        if 'userid' not in session:
            return jsonify({'status': 'failed','message': 'Please login'}), 401
        # reconnect mysql
        mydb.ping(reconnect=True)
        cursor = mydb.cursor(buffered=True)
        userid = session.get('userid')
        ...
        cursor.execute('''
            ...
            values( %s,%s,uuid_to_bin(%s), %s,%s,%s,%s)
        ''', [order_id,payment_id,userid,subtotal,delivery,tax, grand_total ])
        ...
        for item in cart_items:
            ...
            mydb.commit()
            return jsonify({'status': 'success','message': 'Payment verified successfully','payment': {'payment_id': payment_id,'order_id': order_id},'summary': {'subtotal': subtotal,'delivery': delivery,'tax': tax,'grand_total': grand_total},'ordered_items': ordered_items})
    except Exception as e:
        mydb.rollback()
        print('Mysql Error:', str(e))
        return jsonify({'status': 'failed','message': str(e)}), 500
...
def forgotpassword():
    ...
    if count_email[0] == 1:
        reset_link = f"https://react-e-commerce-cart-nest-n3wb.vercel.app/resetpassword/{endata(f_email)}"
